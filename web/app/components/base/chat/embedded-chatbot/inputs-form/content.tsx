...
const InputsFormContent = ({ showTip }: Props) => {
  ...
  const getInputMode = useCallback((variable: string): InputMode => {
    return inputModes[variable] || 'form'
  }, [inputModes])

  // Set input mode for a variable
  const setInputMode = useCallback((variable: string, mode: InputMode) => {
    setInputModes(prev => ({ ...prev, [variable]: mode }))
  }, [])
  ...
  const handleJsonChange = useCallback((variable: string, jsonValue: string) => {
    try {
      const parsed = JSON.parse(jsonValue)
      handleFormChange(variable, parsed)
    }
    catch {
      // Keep the raw string if not valid JSON - will be validated on submit
      handleFormChange(variable, jsonValue)
    }
  }, [handleFormChange])
  ...
  return (
    <div className='space-y-4'>
      {visibleInputsForms.map((form) => {
        const isComplexType = [InputVarType.object, InputVarType.arrayObject, InputVarType.arrayString, InputVarType.arrayNumber, InputVarType.arrayBoolean].includes(form.type as InputVarType)
        const hasChildren = form.children && form.children.length > 0
        const currentMode = getInputMode(form.variable)
        ...
        return (
          <div key={form.variable} className='space-y-1'>
              ...
              <div className='flex h-6 items-center justify-between gap-1'>
                ...
                {isComplexType && hasChildren && (
                  <div className='flex shrink-0 items-center gap-1'>
                    <button
                      type='button'
                      onClick={() => setInputMode(form.variable, 'form')}
                      className={cn(
                    ...
                    <button
                      type='button'
                      onClick={() => setInputMode(form.variable, 'json')}
                      className={cn(
                        'system-xs-medium rounded px-1.5 py-0.5 transition-colors',
                        currentMode === 'json'
                          ? 'bg-components-button-secondary-bg text-text-secondary'
            ...
            {form.type === InputVarType.object && form.children && form.children.length > 0 && currentMode === 'form' && (
            ...
            {form.type === InputVarType.object && form.children && form.children.length > 0 && currentMode === 'json' && (
              <CodeEditor
                value={getJsonValue(inputsFormValue?.[form.variable])}
                language={CodeLanguage.json}
                onChange={v => handleJsonChange(form.variable, v)}
                noWrapper
            ...
            {form.type === InputVarType.arrayString && currentMode === 'form' && (
            ...
            {form.type === InputVarType.arrayString && currentMode === 'json' && (
              <CodeEditor
                value={getJsonValue(inputsFormValue?.[form.variable])}
                language={CodeLanguage.json}
                onChange={v => handleJsonChange(form.variable, v)}
            ...
            {form.type === InputVarType.arrayNumber && currentMode === 'form' && (
              <div className='space-y-2'>
            ...
            {form.type === InputVarType.arrayNumber && currentMode === 'json' && (
              <CodeEditor
                value={getJsonValue(inputsFormValue?.[form.variable])}
                language={CodeLanguage.json}
                onChange={v => handleJsonChange(form.variable, v)}
            ...
            {form.type === InputVarType.arrayBoolean && currentMode === 'form' && (
              <div className='space-y-2'>
            ...
            {form.type === InputVarType.arrayBoolean && currentMode === 'json' && (
              <CodeEditor
                value={getJsonValue(inputsFormValue?.[form.variable])}
                language={CodeLanguage.json}
                onChange={v => handleJsonChange(form.variable, v)}
            ...
            {form.type === InputVarType.arrayObject && form.children && form.children.length > 0 && currentMode === 'form' && (
              <div className='space-y-2'>
            ...
            {form.type === InputVarType.arrayObject && form.children && form.children.length > 0 && currentMode === 'json' && (
              <CodeEditor
                value={getJsonValue(inputsFormValue?.[form.variable])}
                language={CodeLanguage.json}
                onChange={v => handleJsonChange(form.variable, v)}
