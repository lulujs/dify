...
type Props = {
  isShow: boolean
  onClose: () => void
  onApply: (template: string, variables?: PromptVariable[]) => void
}
...
const PromptTemplateModal: FC<Props> = ({
  ...
  const workspaceTemplates: TemplateItem[] = [
    {
      ...
      prompt: '你是一位专业的客户服务代表。请根据客户的问题 {{question}} 提供帮助。\n\n要求：\n1. 保持友好和专业的态度\n2. 快速准确地理解客户需求\n3. 提供清晰的解决方案\n4. 如遇复杂问题，及时升级处理',
    },
    {
    ...
    },
    {
      ...
      prompt: '请为 {{platform}} 平台创作一条社交媒体文案：\n\n主题：{{topic}}\n目标：{{goal}}\n风格：{{style}}\n\n要求：\n1. 符合平台特点\n2. 包含 {{hashtag_count}} 个相关话题标签\n3. 吸引用户互动\n4. 字数限制：{{char_limit}} 字符',
    },
    {
      key: 'bugReporter',
      name: 'Bug 报告生成器',
  ...
  const handleApply = useCallback(() => {
    if (selectedTemplate) {
      // 从编辑后的模板中提取变量
      const varKeys = getVars(editedTemplate)

      // 创建全新的变量列表，替换现有变量
      const newVariables = varKeys.map(key => getNewVar(key, 'string'))

      // 使用编辑后的模板内容和新变量（会清空并替换现有变量）
      onApply(editedTemplate, newVariables)
      hideConfirm()
    }
  ...
  return (
    <>
        ...
        <div className='flex h-[700px] flex-col'>
          ...
          <div className='border-b border-divider-regular px-6 py-4'>
            <div className='text-lg font-semibold text-text-primary'>
              {t('appDebug.promptTemplate.title')}
            </div>
            <div className='mt-1 text-sm text-text-tertiary'>
              {t('appDebug.promptTemplate.description')}
            </div>
          </div>
            ...
            <div className='w-[600px] border-r border-divider-regular'>
              <div className='flex h-full flex-col'>
                ...
                <div className='border-b border-divider-regular px-4 py-3'>
                  <div className='flex items-center justify-between'>
                    ...
                    <div className='flex items-center space-x-1 rounded-lg bg-background-section p-0.5'>
                      ...
                      >
                        {t('appDebug.promptTemplate.mine')}
                      </button>
                      <button
                        className={cn(
                          'rounded-md px-3 py-1.5 text-sm font-medium transition-colors',
                      ...
                      >
                        {t('appDebug.promptTemplate.workspace')}
                      </button>
                    </div>
                    ...
                    <div className='w-64'>
                      <Input
                        ...
                        placeholder={t('appDebug.promptTemplate.searchPlaceholder')}
                        showLeftIcon
                      />
                    </div>
                  </div>
                ...
                <div className='grid grid-cols-12 gap-2 border-b border-divider-regular bg-background-section px-4 py-3 text-xs font-semibold text-text-tertiary'>
                  <div className='col-span-5'>{t('appDebug.promptTemplate.promptName')}</div>
                  <div className='col-span-7'>{t('appDebug.promptTemplate.template')}</div>
                </div>
                    ...
                    <div className='flex h-full items-center justify-center'>
                      <div className='text-center'>
                        <div className='text-sm text-text-tertiary'>
                          {activeTab === 'mine'
                            ? t('appDebug.promptTemplate.noMyTemplates')
                            : t('appDebug.promptTemplate.noResults')}
                        </div>
                      </div>
                    </div>
                  )}
                ...
                <div className='flex items-center justify-between border-t border-divider-regular px-4 py-3 text-sm text-text-tertiary'>
                  <div>
                    {t('appDebug.promptTemplate.pagination', {
                      current: currentPage,
                  ...
                  <div className='flex items-center space-x-2'>
                    ...
                    >
                      {t('appDebug.promptTemplate.previousPage')}
                    </Button>
                    <Button
                      size='small'
                      variant='ghost'
                    ...
                    >
                      {t('appDebug.promptTemplate.nextPage')}
                    </Button>
                  </div>
                </div>
              </div>
            ...
            <div className='flex flex-1 flex-col p-6'>
              <div className='mb-4'>
                <div className='text-sm font-semibold text-text-secondary'>
                  {t('appDebug.promptTemplate.editArea')}
                </div>
              </div>
              <div className='flex-1 overflow-hidden'>
                <div className='relative h-full'>
                  <PromptEditor
                    ...
                    instanceId='prompt-template-editor-config'
                    value={editedTemplate}
                    placeholder={
                      <div className='system-sm-regular text-text-placeholder'>
                        {t('appDebug.promptTemplate.editPlaceholder')}
                      </div>
                    }
      ...
      {isShowConfirm && (
        <Confirm
          title={t('appDebug.promptTemplate.confirmTitle')}
          content={t('appDebug.promptTemplate.confirmMessage')}
          isShow
